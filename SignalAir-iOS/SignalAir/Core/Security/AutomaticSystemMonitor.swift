...
/// 自動系統監控系統 - 取代管理員手動查看安全事件
/// 實現完全自動化的安全威脅檢測、分析和處理
class AutomaticSystemMonitor: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "AutomaticSystemMonitor", qos: .utility)
    ...
    init() {
        setupThreatPatterns()
        print("🛡️ AutomaticSystemMonitor: 自動系統監控系統已初始化")
    }
    ...
    private func executeSecurityScan() -> SecurityScanResult {
        print("🔍 AutomaticSystemMonitor: 執行安全掃描")
        ...
        print("✅ AutomaticSystemMonitor: 安全掃描完成 - 威脅等級: \(overallThreatLevel), 發現威脅: \(detectedThreats.count)個")
    ...
    private func analyzeEventsForThreats(_ events: [SecurityEvent]) -> [SecurityThreat] {
        ...
        // 分析連線速率異常模式
        let connectionRateThreats = detectConnectionRateAnomalies(events)
        detectedThreats.append(contentsOf: connectionRateThreats)
    ...
    private func detectConnectionRateAnomalies(_ events: [SecurityEvent]) -> [SecurityThreat] {
        ...
        for (sourceID, sourceEvents) in eventsBySource {
            ...
            if recentEvents.count > 50 { // 5分鐘內超過50個事件
                let threat = SecurityThreat(
                    id: "connectionRate_\(sourceID)_\(Date().timeIntervalSince1970)",
                    source: sourceID,
                    type: .highConnectionRate,
                    severity: .high,
                    details: "檢測到連線速率異常：5分鐘內\(recentEvents.count)個事件"
                )
    ...
    private func processDetectedThreat(_ threat: SecurityThreat) {
        print("⚠️ AutomaticSystemMonitor: 處理檢測到的威脅 - \(threat.type): \(threat.details)")
        
        // 根據威脅類型自動執行對應措施
        switch threat.type {
        case .highConnectionRate:
            handleHighConnectionRate(threat)
    ...
    private func handleHighConnectionRate(_ threat: SecurityThreat) {
        // 自動啟動連線速率保護
        print("🚫 AutomaticSystemMonitor: 啟動連線速率保護 - 來源: \(threat.source)")
        // 這裡會整合到 ConnectionRateManager 系統
    }
    
    private func handleSuspiciousBehavior(_ threat: SecurityThreat) {
        // 增強對該來源的監控
        print("👁️ AutomaticSystemMonitor: 增強監控可疑行為 - 來源: \(threat.source)")
    }
    
    private func handleUnauthorizedAccess(_ threat: SecurityThreat) {
        // 自動封鎖未授權存取
        print("🔒 AutomaticSystemMonitor: 封鎖未授權存取 - 來源: \(threat.source)")
    }
    
    private func handleNetworkAnomaly(_ threat: SecurityThreat) {
        // 啟動網路診斷
        print("📡 AutomaticSystemMonitor: 啟動網路診斷 - \(threat.details)")
    }
    
    private func handleDataCorruption(_ threat: SecurityThreat) {
        // 啟動資料完整性檢查
        print("🔧 AutomaticSystemMonitor: 啟動資料完整性檢查")
    }
    ...
    private func handleImmediateThreat(_ threat: SecurityThreat) {
        print("🚨 AutomaticSystemMonitor: 處理立即威脅 - \(threat.details)")
        processDetectedThreat(threat)
    }
    ...
    private func logSecurityEvent(_ event: SecurityEvent) {
        ...
        print("\(logPrefix) AutomaticSystemMonitor: [\(event.type.rawValue)] \(event.peerID) - \(event.details)")
    }
    ...
    private func mapEventTypeToThreatType(_ eventType: SecurityEventType) -> ThreatType {
        switch eventType {
        case .connectionRateProtection: return .highConnectionRate
