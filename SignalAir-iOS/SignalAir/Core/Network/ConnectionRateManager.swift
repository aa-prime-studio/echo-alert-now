...
// MARK: - Connection Rate Manager Configuration
struct ConnectionRateManagerConfig {
    ...
    static let `default` = ConnectionRateManagerConfig(
...
class ContentPatternDetector {
    ...
    func checkSuspiciousContent(_ data: Data, from peerID: String) -> (isSuspicious: Bool, shouldBan: Bool) {
        ...
        // 檢查是否為遊戲相關的合法重複訊息
        if isLegitimateGameMessage(data) {
            // 遊戲房間同步、狀態更新等合法重複訊息，跳過可疑內容檢查
    ...
    /// 檢查是否為合法的遊戲訊息（房間同步、狀態更新等）
    private func isLegitimateGameMessage(_ data: Data) -> Bool {
        // 嘗試解析為二進制訊息格式
        do {
            ...
            if message.type == .game {
                ...
                if let messageString = String(data: data, encoding: .utf8) {
                    ...
                    for pattern in legitimateGamePatterns {
                ...
                // 如果無法解析為字串，但確認是遊戲訊息，給予適度放寬
                return true
            }
            
            // 心跳和系統訊息也允許重複
            if message.type == .heartbeat || message.type == .system {
                return true
            }
            ...
            // 解析失敗，檢查是否包含遊戲相關關鍵字
            if let messageString = String(data: data, encoding: .utf8) {
                let gameKeywords = ["room_state", "game", "player", "bingo", "reconnect"]
...
// MARK: - Connection Rate Manager Main Class
// MARK: - ConnectionRateManagerProtocol
// Note: ConnectionRateManagerProtocol is defined in MeshManager.swift

class ConnectionRateManager: ConnectionRateManagerProtocol, ObservableObject {
    // MARK: - Properties
    private let config: ConnectionRateManagerConfig
    ...
    // MARK: - Statistics
    private var stats = ConnectionRateStats()
    
    // MARK: - Initialization
    init(config: ConnectionRateManagerConfig = .default) {
        self.config = config
        startCleanupTimer()
        print("🛡️ ConnectionRateManager initialized with config: \(config)")
    }
    
    // 🔧 FIX: 新增 deinit 方法以正確清理 Timer
    deinit {
        cleanupTimer?.invalidate()
        cleanupTimer = nil
        print("🧹 ConnectionRateManager: deinit 完成，Timer已清理")
    }
    ...
    func shouldBlock(_ message: MeshMessage, from peerID: String) -> Bool {
        ...
        if !checkOverallRateLimit(for: peerID) {
            ...
            logSecurityEvent(
                ...
                severity: .error,
                details: "連線速率保護觸發 - PeerID: \(peerID), 訊息速率超限"
            )
            ...
            if shouldBanPeer(peerID) {
                ...
                logSecurityEvent(
                    ...
                    severity: .critical,
                    details: "封禁節點 - PeerID: \(peerID), 原因: 嚴重超出連線速率, 時長: \(config.banDuration)秒"
                )
            }
    ...
    func shouldAcceptMessage(from deviceID: String, content: Data, size: Int, priority: MessagePriority) -> Bool {
        ...
        let messageType: MeshMessageType = priority == .emergency ? .emergency : .signal
    ...
    /// 取得統計信息
    func getStats() -> ConnectionRateStats {
        return stats
    }
    ...
    func setActive(_ active: Bool) {
        isActive = active
        print("🛡️ ConnectionRateManager \(active ? "enabled" : "disabled")")
    }
    ...
    private func startCleanupTimer() {
        // 🔧 FIX: 使用 weak self 避免強引用循環，並將 Timer 儲存以便清理
    ...
    private func performCleanup() {
        ...
        print("🧹 ConnectionRateManager cleanup completed")
    }
}

// MARK: - Connection Rate Statistics
struct ConnectionRateStats {
