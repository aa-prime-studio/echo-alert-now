...
// 使用 TextKit 的進階版本（更精確的控制）
struct AdvancedLegalTextView: UIViewRepresentable {
    let text: String
    var fontSize: CGFloat = 16
    ...
    class Coordinator: NSObject, NSLayoutManagerDelegate {
        ...
        func layoutManager(_ layoutManager: NSLayoutManager, 
            ...
            guard let textStorage = layoutManager.textStorage else { return true }
            let text = textStorage.string
            
            // 確保索引有效
            guard charIndex < text.count else { return true }
            
            let index = text.index(text.startIndex, offsetBy: charIndex)
            
            // 檢查是否為禁止在行首的標點
            let prohibitedPunctuations = ["。", "，", "、", "；", "：", "？", "！", "）", "」", "』", "》", "〉", "】", "〕", "｝", "％", "‰"]
            
            // 如果下一個字元是禁止的標點，不在此處斷行
            if charIndex < text.count - 1 {
                let nextIndex = text.index(after: index)
                let nextChar = String(text[nextIndex])
                if prohibitedPunctuations.contains(nextChar) {
                    return false
                }
            }
    ...
    private func updateContent(_ textView: UITextView) {
        let paragraphStyle = NSMutableParagraphStyle()
